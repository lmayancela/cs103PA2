...
def to_cat_dict(cat_tuple):
    cat = {'item#':cat_tuple[1], 'amount':cat_tuple[2], 'category':cat_tuple[3], 'date':cat_tuple[4], 'description':cat_tuple[8]}
    return cat
...
class Transaction():
    ...
    def add(self,item):
        con= sqlite3.connect(self.dbfile)
        cur = con.cursor()
        cur.execute("INSERT INTO transactions VALUES(?,?,?,?,?,?,?,?)",(item['item#'],item['amount'],item['category'],'date',item['day'],item['month'],item['year'],item['description']))
        con.commit()
